    ...
    def final_assemble(self, line: str):
        try:
            match self._type:
                ...
                case "C":
                    _, r0, r1 = line.split()
                    return f"{self.opcode}00000{handle_register(r0)}{handle_register(r1, flags_allowed=(self.opcode=='10011'))}"
                case "D":
...
def handle_immediate(immediate: str):
    ...
    if len(immediate) > 8:
        throw_error("Overflow", "Immediate value used on Line {} is too large on Line")
    else:
